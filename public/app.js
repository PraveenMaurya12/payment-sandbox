...
async function runSDKStep(sessionId) {
  ...
  addLog("Launching 3DS SDK…", "info");
  setStatus("pending", "SDK RUNNING…");
  try {
    const threeDS = evervaultSDK.ui.threeDSecure(sessionId);
    threeDS.on("ready", () => {
      el("sdk-modal-notice").classList.add("visible");
      setStatus("challenge", "AUTHENTICATING");
      addLog("SDK ready — complete the challenge", "info");
    });
    threeDS.on("success", async () => {
    ...
    threeDS.mount();
    addLog("Challenge overlay launched", "info");
  } catch (err) {
